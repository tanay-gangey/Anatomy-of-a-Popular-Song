...
```{r}
d<-data.frame(us$Position,us$acousticness)
ag=aggregate(us$acousticness ~ us$Position, d, median)

```

```{r}
ggplot(ag, aes(x = ag$`us$Position`, y =ag$`us$acousticness`) )+ 
  geom_line(aes(color = "Loudness"), size = 1) +ylim(0, 1)
```
